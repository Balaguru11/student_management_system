...
apiRouter.post("/get-student-enrollment-data", (req, res) => {
  ...
  dbcon.query(getStudent, (err, data) => {
    if (err) {
      ...
      console.log(data);
      res.json({ msg: "success", admission_id: data[0].id, student_name: data[0].name, student_mobile: data[0].mobile_number, student_email: data[0].email, academic_year: data[0].academic_year, class_std: data[0].class_std, class_med: data[0].medium, class_sec:data[0].class_section, actual_fee: data[0].actual_fee, earlier_paid: data[0].paying_amount});
    } else {
      console.log(data);
      res.json({ msg: "error", err });
    }
